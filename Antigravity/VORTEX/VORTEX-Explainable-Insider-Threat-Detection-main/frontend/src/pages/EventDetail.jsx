...
const EventDetail = () => {
                    ...
                    <ol key={`list-${elements.length}`} className="space-y-1.5 my-3 ml-1">
                        {listItems.map((item, i) => (
                            <li key={i} className="flex gap-2 text-gray-300">
                                <span className="flex-shrink-0 w-5 h-5 rounded-full bg-red-900/60 text-red-300 flex items-center justify-center text-[10px] font-bold mt-0.5">{i + 1}</span>
    ...
    return (
        <Layout
            title={`Event: ${eventId}`}
            subtitle={`Detailed analysis and SHAP explanation`}
        >
            {/* Back Button */}
            <button
                ...
                <div className="flex items-start justify-between">
                    <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            <div>
                                ...
                                <p className="font-mono text-lg">{eventInfo.event_id}</p>
                            </div>
                            <div>
                                <p className="text-sm text-gray-400 mb-1">User ID</p>
                                <button
                                    onClick={() => navigate(`/user/${eventInfo.user_id}`)}
                                    className="text-lg text-vortex-accent hover:text-vortex-accent-hover"
                                >
                                    {eventInfo.user_id}
                                </button>
                            </div>
                            <div>
                                ...
                                <p className="text-lg">{new Date(eventInfo.timestamp).toLocaleString()}</p>
                            </div>
                            <div>
                    ...
                    <div className="flex items-center space-x-2">
                        {eventInfo.anomaly_flag_truth === 1 ? (
                            <div className="flex items-center space-x-2 bg-red-900 bg-opacity-30 px-4 py-2 rounded-lg">
                                <XCircle size={20} className="text-red-400" />
                                <span className="text-red-200">Confirmed Anomaly</span>
                            </div>
                        ) : (
                            <div className="flex items-center space-x-2 bg-green-900 bg-opacity-30 px-4 py-2 rounded-lg">
                                <CheckCircle size={20} className="text-green-400" />
                                <span className="text-green-200">Normal Behavior</span>
                            </div>
                        )}
                    </div>
                </div>
