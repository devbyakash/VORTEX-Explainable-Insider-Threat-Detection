...
def get_risk_events(
    ...
    elif not date and not user_id:
        # Default: show Medium and High only when browsing all events
...
def get_risk_counts(user_id: Optional[str] = None):
    """
    Returns total event counts, optionally filtered by user.
    ...
    # Per-user breakdown (only if no user_id filter)
    per_user = {}
    if not user_id:
...
@app.get("/simulation/status", summary="Get Simulation Status (Before/After Comparison)")
def get_simulation_status(user_id: Optional[str] = None):
    """
    Returns current event counts per user for before/after injection comparison.
    Call this BEFORE and AFTER injection to see the delta.
    """
    if not data_store.is_loaded():
        raise HTTPException(status_code=503, detail="Service data not loaded")
