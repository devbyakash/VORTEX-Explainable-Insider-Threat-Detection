"""
VORTEX Threat Simulation & Injection Module

Allows targeted simulation of security threats for specific users.
Calculates features and anomaly scores in real-time so the threat 
shows up immediately in detection screens.
"""
...
class ThreatSimulator:
    """
    Handles generation and injection of simulated threat events.
    ...
    def inject_threat(self, user_id: str, scenario_id: str, params: Dict[str, Any], current_df: pd.DataFrame) -> int:
        ...
        raw_events = self._generate_raw_scenarios(user_id, scenario_id, params)
        ...
        if self.model:
            processed_events = self._predict_anomaly_scores(processed_events)
        else:
    ...
    def _generate_raw_scenarios(self, user_id: str, scenario_id: str, params: Dict[str, Any]) -> List[Dict]:
        events = []
        now = datetime.now()
        ...
        if scenario_id == 'data_exfiltration':
            ...
            for i in range(attempts):
                events.append(self._create_raw_base(user_id, now - timedelta(minutes=attempts-i), {
                    ...
                    'admin_action': True if level == 'System' else False,
                    'is_unusual_login': True if i == 0 else False
                }))
    ...
    def _enrich_events(self, events: List[Dict], current_df: pd.DataFrame) -> List[Dict]:
        ...
        for ev in events:
            ...
            # Map Z-Scores for rolling features too
            p_ev['total_files_24h_zscore'] = 2.5 # High signal
            p_ev['avg_upload_24h_zscore'] = 2.0
            p_ev['event_count_24h_zscore'] = 1.0
            
            enriched.append(p_ev)
            
        return enriched

    def _predict_anomaly_scores(self, events: List[Dict]) -> List[Dict]:
        ...
        for i, score in enumerate(scores):
            ...
            # Risk Level categorization (Static thresholds for simulation demo)
            if score > 0.7: events[i]['risk_level'] = 'Critical'
            elif score > 0.4: events[i]['risk_level'] = 'High'
            elif score > 0.1: events[i]['risk_level'] = 'Medium'
