...
import { Filter, Eye, RefreshCw, ArrowUpDown, ArrowUp, ArrowDown, ChevronLeft, ChevronRight, X, Activity, ShieldAlert, AlertTriangle, AlertCircle, CheckCircle } from 'lucide-react';
...
const Alerts = () => {
    ...
    const fetchEvents = useCallback(async ({
        ...
        try {
            ...
            if (srch && !uid) {
                params.search = srch;
                countParams.search = srch;
            }

            const [data, countData] = await Promise.all([
                getRiskEvents(params),
                getRiskCounts(countParams),
            ]);
            setEvents(data);
            setCounts(countData);
        } catch (error) {
    ...
    }, [sortBy, sortOrder, page, pageSize, filters.riskLevel, urlUser, urlDate, filters.search]);
    ...
    useEffect(() => {
        // Skip first render search if it just matches URL
        if (filters.search === (urlSearch || urlUser) && page === 0) return;

        const timer = setTimeout(() => {
            setPage(0);
            fetchEvents({ srch: filters.search, pg: 0 });
        }, 400);
    ...
    return (
        <Layout title="Security Alerts">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                {[
                    { label: 'Total Events', value: counts?.total_events ?? 0, color: '#ffffff', icon: Activity, borderColor: 'border-vortex-accent/20' },
                    { label: 'Critical Risk', value: counts?.by_risk_level?.Critical ?? 0, color: '#fb2c36', icon: ShieldAlert, borderColor: 'border-risk-critical/30' },
                    { label: 'High Risk', value: counts?.by_risk_level?.High ?? 0, color: '#ff6900', icon: AlertTriangle, borderColor: 'border-risk-high/30' },
                    { label: 'Medium Risk', value: counts?.by_risk_level?.Medium ?? 0, color: '#f0b100', icon: AlertCircle, borderColor: 'border-risk-medium/30' },
                    { label: 'Low Risk', value: counts?.by_risk_level?.Low ?? 0, color: '#00c950', icon: CheckCircle, borderColor: 'border-risk-low/30' },
                ].map(({ label, value, color, icon: Icon, borderColor }) => (
                    <div key={label} className={`card py-4 px-5 flex items-start justify-between relative overflow-hidden border-t-2 ${borderColor}`}>
                        <div className="flex flex-col relative z-10">
                            <span className="text-[10px] text-gray-500 uppercase font-black tracking-widest">{label}</span>
                            <span className="text-2xl font-black mt-1" style={{ color: color }}>{value.toLocaleString()}</span>
                        </div>
                        <div className="p-2 bg-gray-900/50 rounded-lg">
                            <Icon size={18} style={{ color: color }} opacity={0.8} />
                        </div>
                        {/* Subtle background glow for Critical */}
                        {label === 'Critical Risk' && value > 0 && (
                            <div className="absolute -bottom-4 -right-4 w-12 h-12 bg-risk-critical/10 blur-2xl rounded-full"></div>
                        )}
                    </div>
                ))}
            </div>
                ...
                <div className="flex items-center justify-between flex-wrap gap-4">
                    <div className="flex items-center space-x-3 flex-1 flex-wrap gap-3">
                        ...
                        <div className="flex items-center gap-2">
                            <span className="text-xs text-gray-500 font-bold uppercase shrink-0">Show</span>
                            <select
                                value={pageSize}
                        ...
                        <input
                            ...
                            className="input-field flex-1 min-w-[150px] max-w-xs py-1.5 px-3 text-sm"
                        />
                    </div>

                    <button
                        onClick={() => fetchEvents()}
                        className="btn-secondary py-1.5 px-3 flex items-center gap-1.5 text-xs h-[38px]"
                        disabled={loading}
                    >
                            ...
                            {events.map((event) => (
                                ...
                                >
                                    ...
                                    <td className="py-3 px-4 text-sm font-semibold">
                                        <span className={
                                            event.risk_level === 'Critical' ? 'text-risk-critical' :
                                                event.risk_level === 'High' ? 'text-risk-high' :
                                                    event.risk_level === 'Medium' ? 'text-risk-medium' :
                                                        'text-risk-low'
                                        }>
